...
export async function sendContact(endpoint, data, fetcher = fetch) {
  if (!/^https:\/\/formspree\.io\/f\/[a-zA-Z0-9]{6,32}$/.test(endpoint))
    throw new Error('The contact form is unavailable. Please use LinkedIn.');
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(
        response.status === 429
          ? 'The form is receiving too many messages. Please try again later or contact me on LinkedIn.'
    ...
    if (error instanceof Error && error.name === 'AbortError')
      throw new Error(
        'The request timed out. Delivery could not be confirmed. Please try again later or use LinkedIn.',
      );
    throw error;
  } finally {
