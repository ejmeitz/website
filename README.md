...
Default build target: `https://ejmeitz.github.io/website/`. The GitHub repository must have **Settings → Pages → Source → GitHub Actions** enabled, and the Actions repository variable `PAGES_ENABLED` set to `true`. Until then, pushes validate and upload the build but skip deployment. GitHub Free requires a public repository for Pages; a private repository requires a supporting paid plan. Deployment does not automatically change DNS or shut down Heroku.
